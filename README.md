...
默认开启本地译文缓存。同一页面使用同一个 Base URL 和模型再次翻译时，扩展会按原文文本节点指纹直接恢复已有译文，只把新增或变化的文本发送给模型。缓存保留最近 20 个页面、总量约 2 MB，可在连接设置中关闭或一键清除。

翻译过程中会动态显示本次输入、输出和总 Token。缓存命中的文本节点不计入本次 Token；网关返回标准 `usage` 时显示精确值，否则显示动态估算值。
...
- 译文缓存只保存原文指纹和中文译文，不保存完整原文；如果页面内容敏感，可关闭或清除缓存。
